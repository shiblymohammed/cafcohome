...
export default function AuthModal({ isOpen, onClose, initialMode = "login", isNewGoogleUser = false }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-6">
      {/* Backdrop */}
      <div 
        className="absolute inset-0 bg-alpha/30 backdrop-blur-md animate-fade-in"
        onClick={onClose}
      />
      
      {/* Modal */}
      <div className="relative bg-white/95 backdrop-blur-2xl w-full max-w-[420px] p-8 sm:p-10 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.3)] border border-white/50 rounded-[2rem] animate-slide-up">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center bg-alpha/5 hover:bg-alpha/10 text-alpha/60 hover:text-alpha rounded-full transition-all duration-300"
          aria-label="Close modal"
        >
        ...
        <div className="text-center mb-8">
          <div className="mx-auto w-12 h-12 flex items-center justify-center bg-alpha/5 rounded-2xl mb-5 shadow-sm">
            <img src="/logo/logo_dark.png" alt="DRAVO" className="h-3 w-auto opacity-80" />
          </div>
          <h2 className="text-2xl font-bold text-alpha tracking-tight mb-2">
            {mode === "login" ? "Welcome Back" : "Create Account"}
          </h2>
          <p className="text-[13px] font-medium text-alpha/50">
            {mode === "login" 
              ? "Enter your credentials to continue"
              : "Join us and discover timeless furniture"
            }
          </p>
        </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 py-3.5 bg-white border border-alpha/10 rounded-2xl shadow-sm hover:shadow-md hover:border-alpha/20 transition-all duration-300 disabled:opacity-50 hover:scale-[1.02] active:scale-[0.98]"
        >
          ...
          <span className="text-[12px] font-bold uppercase tracking-widest text-alpha/80">
        ...
        <div className="flex items-center gap-4 my-6">
          <span className="flex-1 h-[1px] bg-gradient-to-r from-transparent to-alpha/10"></span>
          <span className="text-[10px] font-bold uppercase tracking-widest text-alpha/30">or email</span>
          <span className="flex-1 h-[1px] bg-gradient-to-l from-transparent to-alpha/10"></span>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="space-y-4">
          {error && (
            <div className="bg-red-50/80 border border-red-100 text-red-600 px-4 py-3 rounded-xl text-[13px] font-medium text-center">
              {error}
            </div>
          )}
          ...
          {mode === "signup" && (
            <div className="relative">
              <input
                ...
                className="w-full bg-alpha/5 border border-transparent focus:border-alpha/20 focus:bg-white rounded-2xl px-5 py-3.5 text-[14px] font-medium text-alpha placeholder:text-alpha/40 focus:outline-none transition-all duration-300 shadow-inner"
                required
              />
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-alpha text-white mt-2 py-4 rounded-2xl text-[12px] font-bold uppercase tracking-[0.2em] hover:bg-tango shadow-md hover:shadow-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-[0.98]"
          >
        ...
        <div className="text-center mt-6">
          <p className="text-[13px] font-medium text-alpha/50 mb-2">
            {mode === "login" ? "Don't have an account?" : "Already have an account?"}
          </p>
          <button 
            onClick={switchMode}
            className="inline-flex items-center gap-1.5 text-[12px] uppercase tracking-widest font-bold text-alpha hover:text-tango transition-colors duration-300 group"
          >
